...
def import_fixes_from_file(project_id, file_content):
    try:
        ...
        if isinstance(fixes, list):
            valid_fixes = []
            for fix in fixes:
                if all(key in fix for key in ["phrase1", "phrase2", "fix1", "fix2", "direction"]):
...
def save_fixes(project_id, fixes):
    ...
    for fix in fixes:
        ...
        cursor.execute("""
            INSERT INTO fixes (src_phrase, tgt_phrase, src_fix, tgt_fix, direction, num_occurrences, percentage, project_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        """, (src_phrase, tgt_phrase, src_fix, tgt_fix, direction, num_occurrences, percentage, project_id))
...
def apply_fixes(document_id, fixes):
    start_time = time.time()
    
    print(f"operation:Starting fix application for document {document_id}")
    print('progress:0')
    print(f"🚀 Starting application of {len(fixes)} fixes to document {document_id}")

    print(f"operation:Applying fixes in forward direction")
    print('progress:10')
    print(f"📋 Applying {len(fixes)} fixes to document {document_id}, forward direction")
    ...
    print(f"operation:Applying fixes in reverse direction") 
    print('progress:50')
    print(f"📋 Applying {len(fixes)} fixes to document {document_id}, reverse direction")
    apply_directed_fixes(
        document_id,
        fixes,
        fix_direction=-1
